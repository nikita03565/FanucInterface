...
public class FanucScript : MonoBehaviour
{
    ...
    float[] newCoordWorlds = new float[6];
    float[] diffWorlds = new float[6];
            ...
            if (input.text.Length > 0)
            {
                ...
                if (arr.Length == 6)
                {
                    ...
                    if (mode == 2)
                    {
                        for (int i = 0; i < 6; ++i)
                        {
                            newCoordWorlds[i] = float.Parse(arr[i]);
                        }
                        for (int i = 0; i < 3; ++i)
                        {
                            diffWorlds[i] = newCoordWorlds[i] - worldPos[i];
                        }

                        for (int i = 3; i < 6; ++i)
                        {
                            if (worldPos[i] > 0 && newCoordWorlds[i] < 0)
                            {
                                diffWorlds[i] = (Mathf.Abs(worldPos[i] - newCoordWorlds[i]) > Mathf.Abs(360 - worldPos[i] + newCoordWorlds[i]) ? -(360 - worldPos[i] + newCoordWorlds[i]) : worldPos[i] - newCoordWorlds[i]);
                            }
                            else if (worldPos[i] < 0 && newCoordWorlds[i] > 0)
                            {
                                diffWorlds[i] = (Mathf.Abs(worldPos[i] - newCoordWorlds[i]) > Mathf.Abs(360 + worldPos[i] - newCoordWorlds[i]) ? -(360 + worldPos[i] - newCoordWorlds[i]) : worldPos[i] - newCoordWorlds[i]);
                            }
                            else
                        ...
                        normCoefWorlds = Mathf.Sqrt(diffWorlds[0] * diffWorlds[0] + diffWorlds[1] * diffWorlds[1] 
                            + diffWorlds[2] * diffWorlds[2] + diffWorlds[3] * diffWorlds[3] + diffWorlds[4] * diffWorlds[4]
                            + diffWorlds[5] * diffWorlds[5]);
                        StartCoroutine("MoveWorlds");
                    }       
                }
                else
                {
    ...
    IEnumerator CoordtoServer()
    {
        ...
        net.Sender(RobotCommands.FanucMoving());
        ReadytoSend = true;
    ...
    IEnumerator MoveWorlds()
    {
        Debug.Log(worldPos[0] - newCoordWorlds[0]);
        float[] diff2 = new float[6];

        float error = normCoefWorlds;

        while (error > 1 + speed * 0.02)
        {
            Debug.Log(error); 
            for (int i = 0; i < 6; ++i)
            {
                worldPos[i] += diffWorlds[i] * speed * Time.deltaTime / normCoefWorlds;
            }

            for (int i = 0; i < 6; ++i)
            {
                diff2[i] = newCoordWorlds[i] - worldPos[i];
            }
            error = Mathf.Sqrt(diff2[0] * diff2[0] + diff2[1] * diff2[1] + diff2[2] * diff2[2] +
            diff2[3] * diff2[3] + diff2[4] * diff2[4] + diff2[5] * diff2[5]);
        ...
        worldPos = newCoordWorlds;
        jointAngles = FanucModel.chooseNearestPose(model.InverseTask(ref worldPos), ref jointAngles);
    }
